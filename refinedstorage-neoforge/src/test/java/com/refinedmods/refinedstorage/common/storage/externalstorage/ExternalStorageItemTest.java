...
public final class ExternalStorageItemTest {
    private ExternalStorageItemTest() {
    }

    @GameTest(template = "empty_15x15")
    public static void shouldExposeItems(final GameTestHelper helper) {
    ...
    @GameTest(template = "empty_15x15")
    public static void shouldExtractItem(final GameTestHelper helper) {
        preparePlot(helper, Direction.EAST, (externalStorage, pos, sequence) -> {
            // Arrange
            sequence.thenWaitUntil(networkIsAvailable(helper, pos, network -> insert(helper, network, STONE, 2)));
    ...
    @GameTest(template = "empty_15x15")
    public static void shouldNoLongerExposeItemsWhenExternalBlockIsBroken(final GameTestHelper helper) {
        preparePlot(helper, Direction.EAST, (externalStorage, pos, sequence) -> {
            // Arrange
            sequence.thenWaitUntil(networkIsAvailable(helper, pos, network -> insert(helper, network, STONE, 2)));
